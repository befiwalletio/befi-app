...
class BNB extends Chain {
  ...
  BNB() : super(symbol: Constant.CHAIN_BNB, chain: Constant.CHAIN_BNB) {
    if (Global.E_RPC.BNB != null) {
      this.chainId = Global.E_RPC.BNB.chainId;
      this.url = Global.E_RPC.BNB.url;
    } else {
      this.chainId = '56';
      this.url = 'https://bsc-dataseed1.binance.org';
    }
  }
  ...
  Future<Map<String, dynamic>> buildAccount() async {
    ...
    List<Coin> coins = await DBHelper.create().queryCoins(wid, contract: Constant.CHAIN_BNB, symbol: Constant.CHAIN_BNB);
    if (coins != null) {
      Coin coin = coins[0];
